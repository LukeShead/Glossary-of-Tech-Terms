...
Some characteristics that procedural programming has is the ability to divide different programs into separate tasks and handle them all separatly in order to complete it's function, as well as this it always uses a 'top down' approach which means that the functions are completed from the top of the program and worked down to the bottom in order to complete them all. When procedural programs deal with data, it keeps it moving along the program as it completes it's job. A characteristic that is a negative is how certain techniques are unable to be used when attempting to handle and secure data, this means that it can be limited.
...
Object oriented programming is about having objects within program that can have it's own individual functions, allowing each object to be used differently in the program. By having different objects within the program that all different. One example are AI in programs, they use other interactions in order to run their own, this can also be seen as event-driven with objects.

Some characteristics of object oriented programming are where it has many different techniques in order to help when dealing with data, one of these is Encapsulation, this is where the program takes data and instead of processing it, it keeps it and stores the data safley outside the program. As well as this it has a technique called inheritance, this is where a class that performs a certain task can be inherited by another class in order to keep some of the functions, this improves the amount of reusability that is available. 

#### Event driven
The last is the Event driven programming, this is the form of programming that will constantly look for some form of input from the user, for example an input of a mouse click, or key press. These are programs that require some interactive features in order to be successful, this can include a game, quiz or just a general slide show that requries input to change slides. With this kind of program being the most common as it can keep the user more entertained than others. 
...
When looking at all program paradigms all of the programs have in common the fact that they very similar things, some examples of this are that they all give inputs and output, they all use variables, they all have forms of repitition within them, they all can have decisions within them and all of them use structures of data in order to function and they all handle data within them. This means that each one does the same thing when creating programs. This is where the relationship can come between them, as all of them can be used for the same things. One program can be built to do the same thing however depending on how it is built it can be all 3 of them. Lastly, the biggest relationship between paradigms is that they are all used for the same thing, every program that is made no matter what paradigm it is made from can be used to solve problems, as all program's functions are too solve problems that people have all of them have this relationship. 

Another big relationship feature between these three paradigms is their ablity to change which paradigm within the program, a function can start out as procedural but can become event-driven throughout, this is because a paradigm is how a program is viewed, if a program is procedural until an event is needed to occur to continue, then the perception of the program changes from a procedural to an event-driven program, this means that different paradigms are able to be in the same program.
